...
def parasite(test_case):
    ...
    infect_b = [[ 0 if x == 3 else -1 for x in row] for row in grid]
    # print("infect_initial", infect)
    for row in range(len(grid)):
        for col in range(len(grid[0])):
            if grid[row][col] == 3:
                queue.append([row, col, 0])
                queue_b.append([row, col, 0])
...
def bfs_infection_B(queue, grid, infect):
    while len(queue):
        row, col, time = queue.popleft()
        if row - 1 >= 0 and (infect[row - 1][col] == -1 or time + 1 < infect[row - 1][col]) and grid[row - 1][col] == 1:
            # print("from", row, col, "to", row-1, col, "time", time+1)
        ...
        if col + 1 < len(grid[0]) and (infect[row][col + 1] == -1 or time + 1 < infect[row][col + 1]) and grid[row][col + 1] == 1:
            ...
            queue.append([row, col + 1, time + 1])
            infect[row][col + 1] = time + 1
        if row - 1 >= 0 and col - 1 >= 0 and (infect[row - 1][col-1] == -1 or time + 1 < infect[row - 1][col-1]) and grid[row - 1][col-1] == 1:
            # print("from", row, col, "to", row-1, col, "time", time+1)
            queue.append([row-1, col-1, time + 1])
            infect[row-1][col-1] = time + 1
        if row - 1 >= 0 and col + 1 < len(grid[0]) and (infect[row - 1][col+1] == -1 or time + 1 < infect[row - 1][col+1]) and grid[row - 1][col+1] == 1:
            # print("from", row, col, "to", row-1, col, "time", time+1)
            queue.append([row-1, col+1, time + 1])
            infect[row-1][col+1] = time + 1
        if row + 1 < len(grid) and col - 1 >= 0 and (infect[row + 1][col-1] == -1 or time + 1 < infect[row + 1][col-1]) and grid[row + 1][col-1] == 1:
            # print("from", row, col, "to", row-1, col, "time", time+1)
            queue.append([row+1, col-1, time + 1])
            infect[row+1][col-1] = time + 1
        if row + 1 < len(grid) and col + 1 < len(grid[0]) and (infect[row + 1][col+1] == -1 or time + 1 < infect[row + 1][col+1]) and grid[row + 1][col+1] == 1:
            # print("from", row, col, "to", row-1, col, "time", time+1)
            queue.append([row+1, col+1, time + 1])
            infect[row+1][col+1] = time + 1
